                ...
                <div class="p10">
                    <script type="text/javascript">//<!--
                        function addNode(id){
                            ...
                            loading('#edit');
                            $.get('<?php echo url('admin::dictmanager/catalogAdd');?>', {id:id}, function(data){
                                try{
                                    data = eval('('+data+')');
                                    alert(data.msg);
                                    init();
                        ...
                        function editNode(id){
                            $('#treeview').removeClass('w770').addClass('w380');
                            loading('#edit');
                            $.get('<?php echo url('admin::dictmanager/catalogEdit');?>', {id:id}, function(data){
                                try{
                                    data = eval('('+data+')');
                                    alert(data.msg);
                                    init();
                        ...
                        function delNode(id){
                            if(confirm('确认要删除该分类以及所有子分类吗？')){
                                ...
                                $.get('<?php echo url('admin::dictmanager/catalogDel');?>', {id:id}, function(data){
                                    if(data == 'true'){
                                        alert('删除成功！');
                                        init();
                                    }else{
                        ...
                        var data={};
                    <?php foreach ($catalog as $v): $path_count = count(Q::normalize($v->path));?>
                        data['<?php echo $v->parent_id?>_<?php echo $v->id?>'] = 'text: <?php echo str_replace(';', '%3B', str_replace('%', '%25', $v->name));?><?php if($path_count>1 && $path_count<4):?> <a style="color:#1B5AA2%3B" href="javascript:addNode(<?php echo $v->id?>)%3B">添加子信息</a><?php endif;?><?php if($path_count>2):?> <a style="color:#1B5AA2%3B" href="javascript:delNode(<?php echo $v->id?>)%3B">删除</a><?php endif;?>; hint: <?php echo str_replace('%', '%25', str_replace(';', '%3B', $v->description!=''?$v->description:$v->name));?>; url: javascript:editNode(<?php echo $v->id?>);';
                    <?php endforeach;?>
