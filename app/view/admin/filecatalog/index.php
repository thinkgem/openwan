...
<?php $this->_block('head'); ?>
    ...
    <script type="text/javascript">        
        ...
        function catalog(id){
            ...
            $('#view').show();
            loading('#view');
            $.get('<?php echo url('admin::filecatalog/catalog');?>', {id:id}, function(data){
                $('#view').html(data);
            });
        }
    </script>
            ...
            <div class="box mt10">
                <div class="p10">
                    <ul class="ul" id="leftNav">
                        <?php foreach(Q::ini('appini/nav/left/fileCatalog') as $key => $nav): if ($_app->authorizedUDI($currentUser['roles'], $nav['udi'])):?>
                    ...
                    <div class="p10">
                        <table class="tb">
                            ...
                            <tbody>
                                ...
                                <tr style="cursor:pointer;" onclick="preview(this, '<?php echo url('admin::filecatalog/preview', array('id'=>$v->id));?>', '<?php echo $v->title;?>');" ondblclick="catalog(<?php echo $v->id?>);">
                                    ...
                                    <td nowrap="true"><a href="javascript:" onclick="catalog(<?php echo $v->id?>);">编目</a></td>
                                </tr>
                                    <?php endforeach;?>
                                <?php endif;?>
                            </tbody>
